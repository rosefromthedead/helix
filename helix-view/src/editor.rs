use crate::{
    align_view,
    document::{
        DamageTracker, DocumentSavedEventFuture, DocumentSavedEventResult, Mode, SavePoint,
    },
    graphics::CursorKind,
    info::Info,
    input::KeyEvent,
...
pub type Motion = Box<dyn Fn(&mut Editor, ViewId)>;
...
impl Editor {
    ...
    pub fn apply_motion<F: Fn(&mut Self, ViewId) + 'static>(&mut self, view: ViewId, motion: F) {
        motion(self, view);
        self.last_motion = Some(Box::new(motion));
    }

    pub fn repeat_last_motion(&mut self, view: ViewId, count: usize) {
    ...
    pub fn new_file_from_stdin(
        ...
        let doc_id = self.new_file_from_document(view_id, action, doc);
